...
var Client = exports.Client = function(args){
  ...
  if (args.ssl) {
    ...
    this.ssl = args.ssl;
    if (args.ssl.key) {
      this.ssl.key = fs.readFileSync(args.ssl.key);
    }
    if (args.ssl.cert) {
      this.ssl.cert = fs.readFileSync(args.ssl.cert);
    }
    if (args.ssl.pfx) {
      this.ssl.pfx = fs.readFileSync(args.ssl.pfx);
    }
    if (args.ssl.ca) {
      this.ssl.ca = fs.readFileSync(args.ssl.ca);
    }
  }
};
